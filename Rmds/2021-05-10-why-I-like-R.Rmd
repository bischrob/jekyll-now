...
utah$wins = sum(startsWith(utah$schedule$RESULT,"W"))
prop.table(table(utah$sims))
phoenix = predictRemaining("phoenix")
phoenix$wins = sum(startsWith(phoenix$schedule$RESULT,"W"))
...
```{r}
round(1 - sum(phoenix$sims + phoenix$wins >= utah$sims + utah$wins) / length(phoenix$sims),2)
```
